...
3. Escribe la letra **junta** con los acordes en **grados** inline `[grado]` delante de la
   silaba del cambio. Si una seccion repite una rueda, deja la letra limpia y pon
   `{grid: ...}` al principio de la seccion (ver Reglas).
...
- **Simplificar seccion repetitiva**: quita los acordes inline silaba a silaba y pon
  `{grid: <rueda en grados>}` al inicio de la seccion. Si la seccion es casi constante pero
  tiene **un acorde distinto**, deja el `{grid}` de la rueda base y conserva **solo** ese
  acorde desviado inline en su linea (ejemplo real: outro de *el-secreto-de-las-tortugas* con
  `[#I]`).
...
- **No repitas acordes en cada verso** ni **el estribillo** (`{start_of_chorus}` una vez +
  `{chorus}` para repetir).
...
- **`{grid}` para acordes que se transponen; `{comment}` solo para prosa**. No metas acordes
  dependientes del tono en un `{comment}`: saldrian mal en el bajo.
